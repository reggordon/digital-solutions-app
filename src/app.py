...
def main():
    ...
    with st.sidebar.expander("Filters", expanded=True):
        ...
        # Allow tuning of the fuzzy-title dedupe threshold to reduce false positives/negatives
        # Removed deduplication settings
        # dedupe_threshold = st.slider(
        #     "Deduplicate title similarity",
        #     min_value=0.70,
        #     max_value=0.98,
        #     value=0.85,
        #     step=0.01,
        #     help="Lower to merge more similar titles; raise to be stricter. Tweak if you still see duplicates.",
        # )
        # debug_dedupe = st.checkbox(
        #     "Show dedupe debug",
        #     value=False,
        #     help="Show normalized URLs, normalized titles and fuzzy pairs to help diagnose duplicate groups.",
        # )
    ...
    # Unified header (title, subtitle, meta, and a refresh action)
    st.markdown(
        f"""
...
""",
    ...
    # Deduplicate articles: prefer canonical/clean URL when available, otherwise normalize title
    from urllib.parse import urlparse, urlunparse

    # Simple integration: keep articles as returned by the fetcher (no aggressive dedupe).
    # This keeps the UI predictable and avoids false merging. If you want dedupe later,
    # we can reintroduce a simpler URL-only dedupe or a configurable option.
